...
const updateLogs: UpdateLog[] = [
  {
    ...
    updates: [
      {
        version: 'v3.6',
        title: '관리자 대시보드 + 보안 고도화',
        description: 'Google Analytics 연동 관리자 페이지 및 보안 전면 강화',
        features: [
          '관리자 대시보드 추가 (실시간 접속자, 방문자 통계, 인기 페이지, 기기/지역 분석)',
          'Google Analytics (GA4) 연동',
          '메인 로그인에서 관리자 비밀번호 입력 시 대시보드로 자동 이동',
          '미들웨어 HMAC 실제 서명 검증 (Web Crypto API)',
          'Timing-safe HMAC 비교로 보안 강화',
          '인증 Rate Limiting 추가 (브루트포스 방지)',
          '프록시 API 경로 allowlist 적용 (SSRF 차단)',
          'image-proxy Content-Type 검증 강화',
          '인증 쿠키 세션 단위로 변경 (브라우저 종료 시 만료)',
          '하드코딩 비밀번호 제거 - 환경변수 전용',
        ],
      },
      {
        title: '사이트 리브랜딩',
        features: [
          '사이트 이름 \'접속중 레기온\'으로 변경',
          '디스코드/카카오톡 링크 갱신',
          '메인 페이지 참여 링크 복원',
          '푸터에 아이템매니아/바로템 바로가기 추가',
        ],
      },
    ],
  },
  {
    ...
    updates: [
      {
        version: 'v3.4',
        title: '조율 팁 + 레기온원 필터 + 모바일 최적화',
        features: [
          '조율 팁 페이지 추가 (/tips/tuning)',
          '레기온원 페이지 총합/본캐/부캐 필터 추가',
          '제작 계산기 상세 화면 모바일 최적화',
          '나이 텍스트 형식 수정 + 카톡 참여 표시 제거',
          '제작 등급 하드코딩 제거 - 시트 직접 관리로 전환',
        ],
      },
    ],
  },
  {
    ...
    updates: [
      {
        version: 'v3.3',
        title: '제작 계산기 인벤 DB 연동',
        description: '인벤 DB API로 정확한 아이템 정보 자동 매칭',
        features: [
          '인벤 DB 연동: 완성품 등급 자동 매칭 (시트 등급 부정확 문제 해결)',
          '완성품 상세 정보 표시: 아이템레벨, 착용레벨, 기본 능력치, 슬롯 수',
          '대박 아이템 상세 정보 카드 추가',
          '목록에 아이템레벨(Lv) 컬럼 추가',
        ],
      },
      {
        title: '메인 홈 통계 개선',
        features: [
          '총합/본캐/부캐 필터 버튼 추가 (본캐만 인원·전투력 확인 가능)',
          '평균 아툴(전투점수) 표시 추가',
        ],
      },
      {
        title: '공지사항 전면 개편',
        description: '파티 규칙 재정비 + 사이트·봇 가이드 통합',
        features: [
          '루드라 파티 3종 체계 정리 (확클팟/학원팟/트라이팟)',
          '이모지 참가 체크 방법, 출발 기준 안내 추가',
          '탭 방식 → 한 페이지 스크롤 방식으로 변경',
          '사이트 가이드, 레기온봇 사용법 섹션 추가',
          '모집글 예시 형식 수정',
        ],
      },
    ],
    ...
    updates: [
      {
        version: 'v3.2',
        title: '제작 계산기 추가',
        description: '마족 제작 레시피 검색 + 수익 시뮬레이션',
        features: [
          '구글 시트 기반 제작 레시피 데이터 연동 (400종+)',
          'aeid API 실시간 시세 자동 반영',
          '카테고리별 필터 (갑옷/대장/세공/연금/요리)',
          '등급·성공률·제작비·기대수익 정렬',
          '상세 화면: 재료별 보유량 입력, 단가 수정, 부족 비용 계산',
          '수익 계산: 성공률·대박확률 슬라이더, 수수료 선택, 시나리오별 손익 분석',
        ],
      },
    ],
  },
  ...
  },
  {
    ...
    updates: [
      {
        version: 'v3.0',
        title: '필드보스 타이머 전문가 수준 업데이트',
        description: '점검 리셋 완전 개편 + 관심 보스 기능',
        features: [
          '점검 리셋 완전 개편: 각 보스별 리젠 시간 개별 계산 (30분 보스 → +30분, 6시간 보스 → +6시간)',
          '점검 날짜+시간 입력 가능 (과거 점검도 정확히 입력)',
          '특수 보스(감시자 카이라, 수호신장 나흐마) 점검 리셋에서 제외',
          '직접 시간 입력: [입력] 버튼으로 게임에서 본 남은 시간 직접 입력 (H:MM:SS, MM:SS, MM)',
          '자동 재시작 즉시 실행 (30초 대기 제거)',
          '활성 타이머 전체 삭제 버튼 추가',
        ],
      },
      {
        title: '관심 보스 기능 추가',
        description: '개인별 관심 보스 설정',
        features: [
          '⭐ 버튼으로 관심 보스 등록/해제 (개인 설정, 브라우저 저장)',
          '관심 보스 황금색 테두리 강조',
          '활성 타이머에서 관심 보스 상단 정렬',
          '관심 보스 5분 전 추가 알림',
          '관심 보스 비프음 5번 (일반 3번)',
        ],
      },
      {
        title: 'UI 개선',
        features: [
          '보정 UI 심플화: 복잡한 ±분 버튼 제거, 남은 시간 직접 입력으로 통일',
          '산들바람 상회 초기화 시간 수정 (일요일 자정 → 수요일 05:00)',
        ],
      },
    ],
  },
  {
    ...
    updates: [
      ...
      },
      {
        title: '유튜브 라이브 감지 개선',
        features: [
          '라이브 체크 시간대 최적화 (12시~22시만)',
          '매시 29분/59분 체크 (하루 20회, 쿼터 20%)',
          '라이브 시작 시 자동 전환',
        ],
      },
    ],
